<template>
  <div class="ai-operation-optimization-container">
    <h3 class="page-subtitle">AI运营优化建议</h3>
    ...
    <el-card class="form-card">
      <el-form :model="requestForm" :rules="formRules" ref="formRef" label-width="120px">
        <el-form-item label="业务类型" prop="businessType">
          ...
          <div class="form-tip">可选参数，用于指定优化的业务领域</div>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" @click="submitRequest" :loading="submitting">获取优化建议</el-button>
    ...
    <el-card v-if="responseData" class="result-card" :loading="submitting">
      ...
      <div class="result-section">
        ...
        <div v-if="parsedAnalysis" class="analysis-content">
          ...
          <!-- 优化建议列表 -->
          <div v-if="parsedAnalysis.suggestions && parsedAnalysis.suggestions.length > 0" class="suggestions-list">
            <h5 class="sub-section-title">优化建议</h5>
            <div class="suggestions-container">
              <el-collapse>
                <el-collapse-item
                  v-for="(suggestion, index) in parsedAnalysis.suggestions"
                  :key="index"
                  :title="`${suggestion.category} - ${suggestion.title} (${getPriorityName(suggestion.priority)})`"
                >
...
const submitRequest = async () => {
  ...
  await formRef.value.validate((valid) => {
    ...
    // 调用API
    authApi.getOperationOptimizationSuggest(requestData)
      .then(response => {
      ...
      .catch(error => {
        ...
        ElMessage.error('获取优化建议失败: ' + (error.message || '网络错误'))
      })
      .finally(() => {
...
<style scoped>
.ai-operation-optimization-container {
  padding: 20px 0;
}
