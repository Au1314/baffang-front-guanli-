<template>
  <div class="module-analysis-container">
    <el-card shadow="never" class="analysis-card">
      <template #header>
        <div class="card-header">
          <div class="header-left">
            ...
            <span class="page-title">模块数据分析</span>
          </div>
        </div>
      </template>
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="选择模块" required>
                <el-select v-model="analysisForm.module" placeholder="请选择分析模块" clearable>
                  <el-option label="平台总览" value="platform" />
                  <el-option label="充电站分布" value="stationDistribution" />
                  <el-option label="车辆认证" value="vehicleAudit" />
                  <el-option label="投诉管理" value="complaint" />
                  <el-option label="审核记录" value="auditRecord" />
                  <el-option label="论坛社区" value="forum" />
                  <el-option label="紧急呼叫" value="emergency" />
                  <el-option label="客服工作" value="customerService" />
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="分析问题" required>
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  placeholder="请输入您想分析的问题，例如：投诉处理效率怎么样？有什么可以改进的？"
                />
              </el-form-item>
            </el-col>
            <el-col :span="4" class="form-action-col">
              <el-form-item>
      ...
      <div v-else-if="analysisResult" class="analysis-content">
        ...
        <el-card class="analysis-info-card">
          <div class="analysis-meta">
            <div class="meta-item">
              <span class="meta-label">分析模块：</span>
              <el-tag>{{ getModuleName(analysisResult.module) }}</el-tag>
            </div>
            <div class="meta-item">
      ...
      <div v-else class="error-container">
        ...
        <p class="hint-text">请填写分析参数，点击开始分析获取结果</p>
      </div>
    </el-card>
  </div>
</template>
...
const moduleNames = {
  platform: '平台总览',
  stationDistribution: '充电站分布',
  vehicleAudit: '车辆认证',
  complaint: '投诉管理',
  auditRecord: '审核记录',
  forum: '论坛社区',
  emergency: '紧急呼叫',
  customerService: '客服工作'
}
...
const startAnalysis = async () => {
  // 表单验证
  if (!analysisForm.module) {
    ElMessage.warning('请选择分析模块')
    return
  }
  if (!analysisForm.question) {
  ...
  try {
    loading.value = true
    const data = await authApi.moduleAnalysis(analysisForm)
    analysisResult.value = data
    ElMessage.success('分析完成')
  } catch (error) {
    console.error('获取模块数据分析失败:', error)
